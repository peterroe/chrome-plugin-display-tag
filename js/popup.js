...
// let a = 234
// alert(a)
// document.querySelector('#open_options_page')?.addEventListener('click', () => {
// 	a = 453
// 	executeScriptToCurrentTab(`
// 	const style = document.createElement('style');
// 	style.innerText = "p::before { content: '<p>';}";
// 	alert(123);
...
// 	// 	alert('当前窗口ID：' + tabId);
// 	// 	chrome.tabs.insertCSS(tabId, {file: 'css/test.css'});
// 	// });
// })
...
// 	executeScriptToCurrentTab('document.body.style.backgroundColor="green";')
// });
...
getCurrentMode().then(value => {
	...
	// alert(isOpenDiplayMode)
})

document.querySelector('#switch').addEventListener('click', function() {

	if(isOpenDiplayMode) {
		sendMessageToContentScript({cmd:'closeDisplayMode'}, function(response){});
		isOpenDiplayMode = 0
	} else {
		sendMessageToContentScript({cmd:'openDisplayMode'}, function(response){});
		isOpenDiplayMode = 1
		getCurrentTabId(tabId => {
